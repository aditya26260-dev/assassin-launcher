...
Last active fix in flight: past the two rendering-surface issues above, the embedded JVM now reaches `net.minecraft.client.main.Main` and runs for several seconds before crashing inside `libc.so`'s `strcmp`, called from a function named `pojavInit` inside `libpojavexec.so` (itself called from `glfwInit()`). Root cause, confirmed via the .so's own strings plus Amethyst's real `JREUtils.java`: `pojavInit` reads the environment variable `POJAV_NATIVEDIR` with `getenv()` and passes the result straight into `strcmp` with no null check. Amethyst sets this unconditionally, to `Context.getApplicationInfo().nativeLibraryDir` (its own app's native lib directory, unrelated to the LWJGL natives folder) - this app never set it at all. Fix: added a `nativeLibraryDir` parameter to `NativeBridge.launchEmbeddedJvm` / `jvm_launcher_bridge.cpp`'s `Java_..._launchEmbeddedJvm`, and `setenv("POJAV_NATIVEDIR", ...)` right alongside the existing `LD_LIBRARY_PATH` setup, before `JLI_Launch`. Not yet confirmed on-device - next session should pick up here with a fresh logcat.
